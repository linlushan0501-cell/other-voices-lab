...
assert.match(style, /body\s*\{[^}]*overflow-y:\s*auto/s, "Pages should use normal browser scrolling.");
assert.match(style, /body\[data-step="generate"\]\s*\{[^}]*overflow-y:\s*auto/s, "The generation page should allow normal vertical scrolling.");
assert.match(style, /\.app-shell\s*\{[^}]*width:\s*min\(100%, 1512px\)/s, "The page shell should fit the browser width.");
assert.doesNotMatch(style, /transform:\s*translate\(-50%, -50%\) scale/, "The page shell should not use transform scaling for layout.");
...
assert.match(style, /grid-template-columns:\s*minmax\(140px, 167px\) 20px repeat\(6, minmax\(112px, 1fr\)\)/, "The record matrix columns should stay aligned while fitting the browser width.");
